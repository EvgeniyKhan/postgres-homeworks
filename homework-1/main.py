...
def insert_data_to_sql(filename, tablename):
    try:
        with open(filename, "r", encoding="UTF-8") as file:
            ...
            with psycopg2.connect(host='localhost', database='north', user='postgres', password='Igorevi4') as conn:
                with conn.cursor() as cursor:
                    for row in data:
                        ...
                        cursor.execute(f"INSERT INFORMATION {tablename} VALUES ({', '.join(values_designation)})",
                                       tuple(row_data))
                        row_data.clear()
