...
SELECT AVG(EXTRACT(EPOCH FROM (shipped_date::timestamp - order_date::timestamp)) / (24 * 3600)) AS average_delivery_days
FROM orders
...
SELECT MIN(unit_price) AS min_price, MAX(unit_price) AS unit_price
FROM products
